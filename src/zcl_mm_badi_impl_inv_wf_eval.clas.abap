class ZCL_MM_BADI_IMPL_INV_WF_EVAL definition
  public
  final
  create public .
...
CLASS ZCL_MM_BADI_IMPL_INV_WF_EVAL IMPLEMENTATION.
...
*** New Logic START >>>>>>>>>
    ...
    lv_invoice = is_sap_object_node_type-sont_key_part_1.
    lv_year = is_sap_object_node_type-sont_key_part_2.
    DATA(lv_key) = lv_invoice && lv_year.

* skip workflow if invoice release group is matching with TVARVC
*>> Start
*    SELECT SINGLE a~belnr, a~gjahr, a~bukrs, a~lifnr, b~frgrp
*           FROM rbkp AS a
*           INNER JOIN lfb1 AS b
*           ON   a~lifnr = b~lifnr
*            AND a~bukrs = b~bukrs
*            WHERE a~belnr = @lv_invoice
*              AND a~gjahr = @lv_year
*      INTO @DATA(ls_rbkp_lfb1).
*    IF sy-subrc IS INITIAL.
*      SELECT SINGLE name,low,high FROM tvarvc INTO @DATA(ls_rel_grp)
*        WHERE name = 'ZPS_WFL0005_REL_GROUP'
*          AND low  = @ls_rbkp_lfb1-frgrp.
*      IF sy-subrc IS INITIAL.
*        cv_is_true = abap_false.
*        RETURN.
*      ENDIF.
...
*<< End
    ...
    SELECT SINGLE supplierinvoice,fiscalyear,createdbyuser
      FROM i_supplierinvoice INTO @DATA(ls_inv_hdr)
      WHERE supplierinvoice = @lv_invoice
...
*      DELETE lt_ekpo WHERE loekz IS NOT INITIAL.
    ...
    DATA(lv_inv_total) =  REDUCE #(
                    INIT subtotal = 0
                    FOR ls_item IN lt_inv_item
                    NEXT subtotal  = subtotal  + ls_item-supplierinvoiceitemamount
                ).
...
*    ENDIF.
    ...
    SELECT * FROM zappr_limit                          "#EC CI_BUFFJOIN
      INTO TABLE @DATA(lt_appr_limit).                  "#EC CI_NOWHERE
    IF sy-subrc IS INITIAL.
...
*      WHERE wi_rh_task = 'TS00800577' AND
*         instid = @lv_key.
*    IF sy-subrc IS INITIAL.
    ...
    LOOP AT lt_hier_struc INTO DATA(ls_hier_struc).
      ...
      IF sy-subrc IS INITIAL.
        ...
        IF sy-subrc IS INITIAL.
          ...
          IF  ls_inv_hdr-createdbyuser NE ls_hier_levels-usrid AND
...
*              lv_numc2 = lv_numc2 + 1.
            ...
            IF lv_inv_total GE ls_appr_limit_1-zamount_from
              AND lv_inv_total LE ls_appr_limit_1-zamount_to.
...
*// This check will be applicable only when no approver found
            IF lv_inv_total LE ls_appr_limit_1-zamount_to
...
*    IF lv_current_level = '01' AND lv_cost_user EQ ls_inv_hdr-createdbyuser.
*      cv_is_true = abap_false.
*      RETURN.
*    ENDIF.
*
...
**        IF ls_appr_limit-zamount_from GE lv_inv_total. " KDURAI 11/04/2024
*        IF lv_inv_total GE ls_appr_limit-zamount_from " KDURAI 11/04/2024
*          AND lv_inv_total LE ls_appr_limit-zamount_to. " KDURAI 11/04/2024
*          READ TABLE lt_hier_levels INTO ls_hier_level
*                                    WITH KEY persk = ls_appr_limit-zlevel.
*          IF sy-subrc IS INITIAL
*            AND ls_hier_level-usrid NE ls_inv_hdr-createdbyuser.
*            DATA(lv_valid_last_level) = ls_appr_limit-zlevel.
**            EXIT. " KDURAI 11/04/2024
*          ENDIF.
*        ENDIF.
*      ENDLOOP.
*
...
*        IF lv_inv_total LE ls_appr_limit-zamount_to.
*          READ TABLE lt_hier_levels INTO ls_hier_level
*                                    WITH KEY persk = ls_appr_limit-zlevel.
*          IF sy-subrc IS INITIAL
*            AND ls_hier_level-usrid NE ls_inv_hdr-createdbyuser.
...
*      lv_curr_level_user NE ls_inv_hdr-createdbyuser " Skip if current level user match with PO creator
...
*      ENDIF.
**// Check conditions
*      IF   ( lv_inv_total GE ls_appr_limit-zamount_from AND lv_current_level > lv_emp_level AND lv_current_level = ls_appr_limit-zlevel )
*        OR  ( ( lv_prev_level_user EQ ls_inv_hdr-createdbyuser OR  lv_prev_level_user IS INITIAL ) AND lv_current_level LE lv_valid_last_level ).
*        cv_is_true = abap_true.
*      ENDIF.
*
*    ENDIF.
*******END <<<<<<<<
** << EOC KAD4COB 18/04/2024
