*&---------------------------------------------------------------------*
*& Include          ZFI_COST_ALLO_WRAPPER_F01
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*& Form file_open_dir
*&---------------------------------------------------------------------*
...
FORM schedule_jobs .
  ...
  SORT gt_data BY skostl.

  DELETE ADJACENT DUPLICATES FROM gt_data COMPARING skostl.
*  DESCRIBE TABLE gt_data LINES DATA(lv_totcnt).

  SELECT *
    FROM tvarvc
    INTO TABLE @DATA(lt_tvarcnt)
   WHERE name EQ 'ZFI_COST_ALLOCATION_TVAR'.

  READ TABLE lt_tvarcnt
  INTO DATA(ls_cnt)
  WITH KEY name = 'ZFI_COST_ALLOCATION_TVAR'
           type = 'S'
           sign = 'I'
           opti = 'BT'.
  ...
  LOOP AT gt_data INTO gs_data.
    lv_cnt = lv_cnt + 1.
    CONCATENATE gs_data-skostl '_' gs_data-descr INTO lv_jobname.
    CALL FUNCTION 'JOB_OPEN'
      ...
      EXCEPTIONS
        ...
        OTHERS           = 4.
    IF sy-subrc EQ 0.
      SUBMIT zfi_cost_allocation WITH p_bpath = p_bpath
                                 WITH p_rldnr = p_rldnr
                                 WITH p_gjahr = p_gjahr
                                 WITH p_monat = p_monat
                                 WITH s_blart IN s_blart
                                 WITH s_budat IN s_budat
                                 WITH p_kostl = gs_data-skostl
                                 WITH p_bl_ab = p_bl_ab
                                 WITH p_budat = p_budat
                                 WITH p_bldat = p_bldat
                                 WITH p_email IN p_email
                                 WITH p_bg = abap_true
                                 USER sy-uname
                                 VIA JOB lv_jobname
                                 NUMBER lv_jobcount
...
* Implement suitable error handling here
    ...
    gs_jobs-jobname = lv_jobname.
    gs_jobs-jobcount = lv_jobcount.
    APPEND gs_jobs TO gt_jobs.
    CLEAR:gs_jobs.

    IF lv_cnt EQ ls_cnt-low.
      DO.
        SELECT jobname,
               jobcount
          FROM tbtco
          INTO TABLE @DATA(lt_jobs)
           FOR ALL ENTRIES IN @gt_jobs
         WHERE jobname EQ @gt_jobs-jobname
           AND jobcount EQ @gt_jobs-jobcount
           AND status EQ 'F'.
        IF sy-subrc EQ 0.
          DESCRIBE TABLE lt_jobs LINES DATA(lv_finjobs).
          lv_cnt = lv_cnt - lv_finjobs.
          LOOP AT lt_jobs INTO DATA(ls_finjob).
            DELETE gt_jobs WHERE jobname = ls_finjob-jobname
                             AND jobcount = ls_finjob-jobcount.
          ENDLOOP.
          REFRESH:lt_jobs.
          EXIT.
        ELSE.
          WAIT UP TO ls_wait-low SECONDS.
        ENDIF.
      ENDDO.
    ENDIF.
...
FORM archive_file .
  ...
  DATA:lv_archive   TYPE sapb-sappfad,
       lv_str       TYPE xstring,
       lv_new(2000) TYPE c.
